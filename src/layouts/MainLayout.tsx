...
import { C_AuthModal } from 'src/features/lkPage/C_AuthModal'
import Image from 'next/image'
import { Icon } from 'shared/uikit/Icon'
import { useRouter } from 'next/router'
import { Button } from 'shared/uikit/Button'
import UnsubscribeModalS from 'shared/components/UnsubscribeModalS/UnsubscribeModalS'
import InfoModalS, { EModalVariant } from 'shared/components/InfoModalS/InfoModalS'
import CallbackButton from 'shared/components/CallbackButton/CallbackButton'
import { useIntersectionObserver } from 'react-intersection-observer-hook'
import { CookiesNotification } from 'shared/components/CookiesNotification/CookiesNotification'
import { CookieValueTypes, getCookie } from 'cookies-next'
import { ICommonAttributes } from 'shared/services/pageData/common/common.interface'
import { IPropertyServiceResponse } from 'shared/services/pageData/propertyService/propertyService.interface'
import PropertyServicesModal from 'shared/components/PropertyServiceModal/PropertyServicesModal'
...
export interface IPropertyService {
  ...
  propertyServiceState: { isOpen: boolean; openProperty: (isOpen: boolean) => void }
}
        ...
        <div className="header">
          <div className="header__content">
            ...
            <div className="header__control">
              ...
              {/* {isDesktop && (
                <div
                  className={`header__icon header__icon_style_user ${
                    isOpen && clientWidth >= tabletWidth ? `isOpen` : ``
                  }`}
                  onClick={() =>
                    !isLogin ? setIsOpen((prev) => !prev) : router.push('/lk/favorites')
                  }>
                  <div data-cy="user" className={classes.userWrapper}>
                    {isOpen && clientWidth >= tabletWidth && <Icon name="activeUser" />}
                    {isOpen && clientWidth < tabletWidth && (
                      <Icon name="close" s="24" color="neutrals-white" />
                    )}
                    {!isOpen && <Icon name="user" />}
                  </div>
                </div>
              )} */}
              <div
                ...
                {!isDisplayHeaderMenu && <Icon name="burger" />}
                {isDisplayHeaderMenu && <Icon name="close" />}
              </div>
            </div>
          </div>
          ...
          <div className="footer">
            <div className="footer__header">
              ...
              <div className="footer__header-i">
                ...
                <div className="footer__button">
                  <Button
                    href={`${common.block_footer.follow_us.button.link}`}
                    s="large"
                    variant="whiteStroke"
